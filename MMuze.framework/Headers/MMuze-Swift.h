...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="MMuze",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
/// A button object that allows starting and stopping of voice based search by MMuze. The button can be placed and
/// customized anywhere within the app. When pressed, recording of user input from device microphone will begin.
/// Pressing again will stop the recording and submit the search.
/// NOTE: Using this button assumes permissions have been granted by the user to use the microphone and speech
/// recognition features of iOS. In the event these have NOT been given, the button will do nothing so as not
/// to prevent future permissions requests.
SWIFT_CLASS("_TtC5MMuze23MMuzeConversationButton")
...
/// :nodoc:
- (void)sendActionsForControlEvents:(UIControlEvents)event;
@end

@class NSBundle;

/// An overlay view that covers the complete flow of MMuze search experience through conversation. The view presents
/// as a modal with conversation interactions including selection, text and voice search within a contextual conversation.
/// To show the view simply call the class method <code>show</code> and the conversation view will display as a modal until it is dismissed.
/// Dismissing and re-opening the conversation view clears it’s context and starts a new.
...
@interface MMuzeConversationView (SWIFT_EXTENSION(MMuze)) <UITableViewDataSource, UITableViewDelegate>
/// :nodoc:
...
@interface MMuzeConversationView (SWIFT_EXTENSION(MMuze)) <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
/// :nodoc:
...
/// :nodoc:
/// A class used to draw a wave form with sliding-window bar behavior. Each bar represents power level of
/// the sound.
SWIFT_CLASS("_TtC5MMuze12WaveFormView")
